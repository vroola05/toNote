...
export class TabMenu {
    ...
    constructor(labels:Map<string, string>, classes:string|undefined){
		...
		this.domTabMenu.appendChild(domTabMenuContainer);
    ...
    public get() : HTMLElement{
        return this.domTabMenu;
    }

	public setChild(child:TabMenu){
		this.child = child;
	}
	...
	public back(){
		if ( this.domTabMenu.classList.contains("active")) {
			this.hide();
    ...
    public hide() : void {
		...
		this.domTabMenu.classList.remove("active");
		this.domTabMenu.classList.remove("stack");
    }
    ...
    public show() : void {
		...
		this.domTabMenu.classList.add("active");
		if ( ! this.domTabMenu.classList.contains("stack")) {
			this.domTabMenu.classList.add("stack");
		}
    }
     ...
     * @param color 
     */
    public addItem(name:string, identifier:number, color:string) : void{
        if (this.domTabMenu.classList.contains("hidden")) {
			this.domTabMenu.classList.remove("hidden");
		}
		...
		item.addEventListener("click", function(e) {
			...
			self.domTabMenu.classList.remove("active");
			if(self.child != null){
				self.child.show();
			}

			self.click(e, Number(this.getAttribute("identifier")));
		});
    ...
    private setMenuItemActive(identifier:number) : void{
        var list = this.domItemList.children;
        for (var i = 0; i < list.length; i++) {
          if(list[i].getAttribute("identifier") == identifier.toString()){
